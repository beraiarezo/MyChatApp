...
{"is_banned","bool","boolean",1,0,0,1},
{"avatar","std::vector<char>","bytea",0,0,0,0}
};
...
Users::Users(const Row &r, const ssize_t indexOffset) noexcept
{
    if(indexOffset < 0)
    {
        ...
        {
            isBanned_=std::make_shared<bool>(r["is_banned"].as<bool>());
        }
        if(!r["avatar"].isNull())
        {
            auto str = r["avatar"].as<std::string_view>();
            if(str.length()>=2&&
                str[0]=='\\'&&str[1]=='x')
            {
                avatar_=std::make_shared<std::vector<char>>(drogon::utils::hexToBinaryVector(str.data()+2,str.length()-2));
            }
        }
    }
    else
    {
        ...
        if(!r[index].isNull())
        {
            isBanned_=std::make_shared<bool>(r[index].as<bool>());
        }
        index = offset + 5;
        if(!r[index].isNull())
        {
...
Users::Users(const Json::Value &pJson, const std::vector<std::string> &pMasqueradingVector) noexcept(false)
{
    ...
    if(!pMasqueradingVector[4].empty() && pJson.isMember(pMasqueradingVector[4]))
    {
        ...
        {
            isBanned_=std::make_shared<bool>(pJson[pMasqueradingVector[4]].asBool());
        }
    }
...
Users::Users(const Json::Value &pJson) noexcept(false)
{
    ...
    if(pJson.isMember("is_banned"))
    {
        ...
        {
            isBanned_=std::make_shared<bool>(pJson["is_banned"].asBool());
        }
    }
...
const bool &Users::getValueOfIsBanned() const noexcept
{
...
const std::shared_ptr<bool> &Users::getIsBanned() const noexcept
{
    return isBanned_;
}
void Users::setIsBanned(const bool &pIsBanned) noexcept
{
    isBanned_ = std::make_shared<bool>(pIsBanned);
    dirtyFlag_[4] = true;
}
...
Json::Value Users::toJson() const
{
    ...
    if(getIsBanned())
    {
        ret["is_banned"]=getValueOfIsBanned();
    }
    else
    {
    ...
    const std::vector<std::string> &pMasqueradingVector) const
{
    ...
    if(pMasqueradingVector.size() == 6)
    {
        ...
        if(!pMasqueradingVector[4].empty())
        {
            if(getIsBanned())
            {
                ret[pMasqueradingVector[4]]=getValueOfIsBanned();
            }
            else
            {
...
std::vector<Messages> Users::getMessages(const DbClientPtr &clientPtr) const {
    static const std::string sql = "select * from messages where sender_id = $1";
    Result r(nullptr);
    {
